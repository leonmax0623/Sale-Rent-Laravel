...
class Hotel extends Bookable
{
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail .= config('hotel.hotel_route_prefix') . $property_type_path . "/" . $this->uid . "/" . $this->slug;
        if (!empty($param)) {
    ...
    public function roomsWithoutTranslation()
    {
    ...
    public static function search(Request $request)
    {
        ...
        if ($request->query('location') && !($request->query('postal_code') && $request->query('state') && $request->query('city') && $request->query('district') && $request->query('street'))) {
            ...
            if($request_location) {
                $model_hotel->where(function($query) use ($request_location) {
        ...
        if ($request->query('autocomplete_formatted')) {
            //$model_hotel->where('autocomplete', $request->query('autocomplete_formatted'));
        }
        if ($request->query('postal_code')) {
            $model_hotel->where('postal_code', $request->query('postal_code'));
        }
        if ($request->query('state')) {
            $model_hotel->where('state', $request->query('state'));
        }
        if ($request->query('city')) {
            $model_hotel->where('city', $request->query('city'));
        }
        if ($request->query('district')) {
            $model_hotel->where('district', $request->query('district'));
        }
        if ($request->query('street')) {
            $model_hotel->where('street', $request->query('street'));
        }
        if ($request->query('property_type')) {
            $model_hotel->where('property_type_id', $request->query('property_type'));
        }
        if ($request->query('sale_price_id')) {
            ...
            if($sale_price) {
                $raw_sql_min_max = "(  bc_hotels.price >= ? )
                                AND (  bc_hotels.price <= ? )";
                $model_hotel->WhereRaw($raw_sql_min_max,[$sale_price->start, $sale_price->end]);
            }
        }
        if ($request->query('adults')) {
            $model_hotel->whereHas('rooms', function ($query) use ($request) {
                $query->where('adults', '>=', $request->query('adults'));
            });
        }
        if ($request->query('children')) {
            $model_hotel->whereHas('rooms', function ($query) use ($request) {
                $query->where('children', '>=', $request->query('children'));
            });
        }
        ...
        if($request_date_period) {
            $model_hotel->whereHas('rooms', function ($query) use ($request_date_period) {
                $query->whereHas('dates', function ($query) use ($request_date_period) {
