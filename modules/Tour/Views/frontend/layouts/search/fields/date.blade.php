        ...
        <div class="row u-datepicker form-date-search">
            ...
            <div class="col-md-6">
                <div class="main_search_wrap input-group flex-nowrap">
                    <span class="main_search_label scroll_body check-in_name"> {{ $first }}</span>
                    <input type="hidden" class="check-in-input" value="{{Request::query('start',display_date(strtotime("today")))}}" name="start">
                    <input type="text" class="scroll_body main_search_input check-in" readonly placeholder="{{ __('Select Dates') }}" value="">
                </div>
            </div>
            <div class="col-md-6 mt-md-0 mt-2">
                <div class="main_search_wrap input-group flex-nowrap">
                    <span class="main_search_label scroll_body check-out_name">{{ $second }}</span>
                    <input type="hidden" class="check-out-input" value="{{Request::query('start',display_date(strtotime("today")))}}" name="end">
                    <input type="text" class="scroll_body main_search_input check-out" readonly placeholder="{{ __('Select Dates') }}" value="">
                </div>
            </div>
        </div>
    </div>
