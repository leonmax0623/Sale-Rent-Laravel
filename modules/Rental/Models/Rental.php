...
class Rental extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bc_rentals';
    public $type = 'rental';
    public $moduleSlug = 'rental';
    public $checkout_booking_detail_file       = 'Rental::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Rental::frontend/booking/detail-modal';
    public $set_paid_modal_file                = 'Rental::frontend/booking/set-paid-modal';
    public $email_new_booking_file             = 'Rental::emails.new_booking_detail';
    public $availabilityClass = RentalDate::class;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->spaceDateClass = RentalDate::class;
    ...
    {
        $meta['seo_title'] = __("Search for Rentals");
        if (!empty($title = setting_item_with_lang("property_rental_page_list_seo_title",false))) {
            $meta['seo_title'] = $title;
        }else if(!empty($title = setting_item_with_lang("property_rental_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("property_rental_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        }else if(!empty($title = setting_item("property_rental_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("property_rental_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("property_rental_page_list_seo_share");
        $meta['full_url'] = url(config('space.rental_route_prefix'));
        return $meta;
    }
    ...
    public function property_type(){
        return $this->hasOne(Terms::class, 'id', 'property_type_id');
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $property_type_slug = ($this->property_type && $this->property_type->slug) ? $this->property_type->slug : null;
        $property_type_path = ($property_type_slug) ? "/" . $property_type_slug : '';
        
        if( request()->input('lang') && !is_default_lang(request()->input('lang')) ) {
            $urlDetail = request()->input('lang') . '/';
        } else {
            $urlDetail = app_get_locale(false, false, '/');
        }
        
        $urlDetail .= config('space.rental_route_prefix') . $property_type_path . "/" . $this->uid . "/" . $this->slug;
        if(!empty($param)){
    ...
    public static function getLinkForPageSearch( $locale = false , $param = [] ){
        ...
        return url(app_get_locale(false , false , '/'). config('space.rental_route_prefix')."?".http_build_query($param));
    }
    ...
    {
        return url(route('rental.admin.edit',['id'=>$this->id]));
    }

    public function setCountryAttribute($value)
    {
        $this->attributes['country'] = $this->field_country;
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('property_rental_booking_buyer_fees'))) {
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('property_rental_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("property_rental_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("property_rental_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("property_rental_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("property_rental_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("property_rental_review_stats", []))) {
    ...
    public function getScoreReview()
    {
        ...
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $space_id, function () use ($space_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $space_id)->where('object_model', "rental")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])->where('object_id', $this->id)->where('object_model', 'space')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('property_rental_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Rentals", ['number' => $number]);
        }
        return __(":number Rental", ['number' => $number]);
    }
    ...
    public static function isEnable(){
        return setting_item('property_rental_disable') == false;
    }

    public function isDepositEnable(){
        return (setting_item('property_rental_deposit_enable') and setting_item('property_rental_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('property_rental_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('property_rental_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('property_rental_deposit_fomular','default');
    }
    ...
    {
        $model_space = parent::query()->select("bc_rentals.*");
        $model_space->where("bc_rentals.status", "publish");
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if(!empty($location)){
                $model_space->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_rentals.location_id')
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bc_rentals.sale_price,0) > 0 and bc_rentals.sale_price >= ? ) OR (IFNULL(bc_rentals.sale_price,0) <= 0 and bc_rentals.price >= ? ) ) 
                            AND ( (IFNULL(bc_rentals.sale_price,0) > 0 and bc_rentals.sale_price <= ? ) OR (IFNULL(bc_rentals.sale_price,0) <= 0 and bc_rentals.price <= ? ) )";
            $model_space->WhereRaw($raw_sql_min_max,[$pri_from,$pri_from,$pri_to,$pri_to]);
        }
        
        if ($request->query('location') && !($request->query('postal_code') && $request->query('state') && $request->query('city') && $request->query('district') && $request->query('street'))) {
            $request_location = $request->query('location');
            $request_location = str_replace(',', ' ', $request_location);
            $request_location = str_replace('    ', ' ', $request_location);
            $request_location = str_replace('   ', ' ', $request_location);
            $request_location = str_replace('  ', ' ', $request_location);
            $request_location = explode(' ', $request_location);
            if($request_location) {
                $model_space->where(function($query) use ($request_location) {
                    foreach($request_location as $key => $value) {
                        if(!$value) {
                            continue;
                        }
                        $query->orWhere('postal_code', 'LIKE', '%' . $value . '%')
        ...
        if ($request->query('autocomplete_formatted')) {
            //$model_space->where('autocomplete', $request->query('autocomplete_formatted'));
        }
        if ($request->query('postal_code')) {
            $model_space->where('postal_code', $request->query('postal_code'));
        }
        if ($request->query('state')) {
            $model_space->where('state', $request->query('state'));
        }
        if ($request->query('city')) {
            $model_space->where('city', $request->query('city'));
        }
        if ($request->query('district')) {
            $model_space->where('district', $request->query('district'));
        }
        if ($request->query('street')) {
            $model_space->where('street', $request->query('street'));
        }
        if ($request->query('property_type')) {
            $model_space->where('property_type_id', $request->query('property_type'));
        }
        if ($request->query('sale_price_id')) {
            $sale_price = RentalPrice::where('id', $request->query('sale_price_id'))->first();
            if($sale_price) {
                $raw_sql_min_max = "( (IFNULL(bc_rentals.sale_price,0) > 0 and bc_rentals.sale_price >= ? ) OR (IFNULL(bc_rentals.sale_price,0) <= 0 and bc_rentals.price >= ? ) ) 
                                AND ( (IFNULL(bc_rentals.sale_price,0) > 0 and bc_rentals.sale_price <= ? ) OR (IFNULL(bc_rentals.sale_price,0) <= 0 and bc_rentals.price <= ? ) )";
                $model_space->WhereRaw($raw_sql_min_max,[$sale_price->start, $sale_price->start, $sale_price->end, $sale_price->end]);
            }
        }
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_space->join('bc_rental_term as tt', 'tt.target_id', "bc_rentals.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_rentals.review_score >= ? AND bc_rentals.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_space->leftJoin('bc_rental_translations', function ($join) {
                    $join->on('bc_rentals.id', '=', 'bc_rental_translations.origin_id');
                });
                $model_space->where('bc_rental_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_space->where('bc_rentals.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_space->orderByRaw("POW((bc_rentals.map_lng-?),2) + POW((bc_rentals.map_lat-?),2)",[$lgn,$lat]);
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_rentals.sale_price, 0 ) > 0 THEN bc_rentals.sale_price ELSE bc_rentals.price END AS tmp_min_price";
        ...
        $model_space->groupBy("bc_rentals.id");
        ...
        if(!empty($request->query('limit'))){
            ...
            $limit = !empty(setting_item("property_rental_page_limit_item"))? setting_item("property_rental_page_limit_item") : 9;
        }
    ...
    public function dataForApi($forSingle = false){
        ...
        if($forSingle){
            ...
            $data['booking_fee'] = setting_item_array('property_rental_booking_buyer_fees');
    ...
    {
        return "\Modules\Rental\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("rental")
            ]
        ];
    }
    public static function getBookingType(){
        return setting_item('property_rental_booking_type','by_day');
    }

    static public function getFormSearch()
    {
        $search_fields = setting_item_array('property_rental_search_fields');
