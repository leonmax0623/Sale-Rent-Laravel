...
@if(is_default_lang())
<div class="panel">
    <div class="panel-title"><strong>{{__("Property Price")}}</strong></div>
    <div class="panel-body">
        @if(is_default_lang())
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Price for the Rental of Your Property in USD ($/USA)")}}</label>
                        <input type="number" step="any" min="0" name="price" class="form-control" value="{{$row->price}}" placeholder="{{__("Property Price")}}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Discount: Price for the Rental of Your Property in USD ($/USA)")}}</label>
                        <input type="number" step="any" name="sale_price" class="form-control" value="{{$row->sale_price}}" placeholder="{{__("Rental Price of the Property at a Discount ")}}">
                        <span><i>{{__("Filled in as desired. Discount percentage will be calculated automatically. The price of the property must be below the regular price.")}}</i></span>
                    </div>
                </div>
                {{--<div class="col-lg-6">
                    <div class="form-group">
