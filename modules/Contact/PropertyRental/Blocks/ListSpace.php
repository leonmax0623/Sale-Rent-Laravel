<?php
namespace Modules\PropertyRental\Blocks;
...
class ListSpace extends BaseBlock
{
        ...
        $this->setOptions([
            'settings' => [
                ...
                [
                    'id'           => 'custom_ids',
                    'type'         => 'select2',
                    'label'        => __('List Rental by IDs'),
                    'select2'      => [
                        'ajax'     => [
                            'url'      => route('rental.admin.getForSelect2'),
                            'dataType' => 'json'
                        ],
                        'width'    => '100%',
                        'multiple' => "true",
                    ],
                    'pre_selected' => route('rental.admin.getForSelect2', [
                        'pre_selected' => 1
                    ])
                ]
            ],
    ...
    public function content($model = [])
    {
        ...
        return view('PropertyRental::frontend.blocks.list-space.'.$model['style'], $data);
    }
    ...
    public function query($model){
        $model_space = Rental::select("bc_rentals.*")->with(['location','translations','hasWishList']);
        ...
        {
            $model_space->where('bc_rentals.is_featured',1);
        }

        if(!empty( $model['custom_ids'] )){
            $model_space->whereIn("bc_rentals.id",$model['custom_ids']);
        }

        $model_space->orderBy("bc_rentals.".$model['order'], $model['order_by']);
