...
class Space extends Bookable
{
    ...
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bc_spaces';
    public $type = 'space';
    public $moduleSlug = 'vacation';
    ...
    public function dates(){
        return $this->hasMany($this->spaceDateClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail .= config('space.space_route_prefix') . $property_type_path . "/" . $this->uid . "/" . $this->slug;
        if(!empty($param)){
    ...
    {
        $model_space = parent::query()->select("bc_spaces.*");
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bc_spaces.sale_price,0) > 0 and bc_spaces.sale_price >= ? ) OR (IFNULL(bc_spaces.sale_price,0) <= 0 and bc_spaces.price >= ? ) ) 
                            AND ( (IFNULL(bc_spaces.sale_price,0) > 0 and bc_spaces.sale_price <= ? ) OR (IFNULL(bc_spaces.sale_price,0) <= 0 and bc_spaces.price <= ? ) )";
        ...
        if ($request->query('adults') || $request->query('children')) {
            $model_space->where('max_guests', '>=', (int)$request->query('adults') + (int)$request->query('children'));
        }
        
        
        $request_date_period = [];
        if($request->query('start') && $request->query('end')) {
            $request_date_period = CarbonPeriod::create(new Carbon($request->query('start')), new Carbon($request->query('end')));
        }
        if($request_date_period) {
            $model_space->whereHas('dates', function ($query) use ($request_date_period) {
                $query->whereIn('start_date', $request_date_period->toArray())->whereIn('end_date', $request_date_period->toArray());
            }, '>=', count($request_date_period));
        }
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_space->join('bc_space_term as tt', 'tt.target_id', "bc_spaces.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_spaces.review_score >= ? AND bc_spaces.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_space->leftJoin('bc_space_translations', function ($join) {
                    $join->on('bc_spaces.id', '=', 'bc_space_translations.origin_id');
                });
                $model_space->where('bc_space_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_space->where('bc_spaces.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_space->orderByRaw("POW((bc_spaces.map_lng-?),2) + POW((bc_spaces.map_lat-?),2)",[$lgn,$lat]);
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_spaces.sale_price, 0 ) > 0 THEN bc_spaces.sale_price ELSE bc_spaces.price END AS tmp_min_price";
