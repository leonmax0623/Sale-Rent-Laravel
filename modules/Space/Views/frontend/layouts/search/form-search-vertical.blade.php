<div class="sidebar border border-color-1 rounded-xs">
    <div class="p-4 mb-1">
        <form action="{{ route("space.search") }}" name="space_search_form" class="bravo_form" method="get">
            ...
            @if(!empty($search_fields))
                @foreach($search_fields as $field)
                    ...
                    @switch($field['field'])
                        @case ('deal_type')
                            @include('Space::frontend.layouts.search.fields.deal_type', ['title' => $field['title']])
                        @break
                        @case ('service_name')
                        ...
                        @break
                        @case ('from_where')
                        @include('Space::frontend.layouts.search.fields.from-where', ['title' => $field['title']])
                        @break
                        @case ('to_where')
                        @include('Space::frontend.layouts.search.fields.to-where', ['title' => $field['title']])
                        @break
                    @endswitch
                @endforeach
            @endif
            <div class="text-center">
                <button id="space_search_button" type="submit" class="btn btn-primary height-60 w-100 font-weight-bold mb-xl-0 mb-lg-1 transition-3d-hover"><i class="flaticon-magnifying-glass mr-2 font-size-17"></i>Search</button>
            </div>
        </form>
    </div>
</div>
