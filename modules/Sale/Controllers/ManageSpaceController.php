...
class ManageSpaceController extends FrontendController
{
    ...
    public function manageSpace(Request $request)
    {
        ...
        $data = [
            ...
            'page_title'         => __("Manage Sales"),
        ];
        return view('Sale::frontend.manageSpace.index', $data);
    }
    ...
    {
        $this->checkPermission('sale_delete');
        $user_id = Auth::id();
        ...
        return redirect(route('sale.vendor.recovery'))->with('success', __('Restore space success!'));
    }
    ...
    public function createSpace(Request $request)
    {
        ...
        $data = [
            ...
            'page_title'         => __("Create Sales"),
        ];
        return view('Sale::frontend.manageSpace.detail', $data);
    }
    ...
    public function store( Request $request, $id )
    {
        ...
        if($id>0){
            ...
            if (empty($row)) {
                return redirect(route('sale.vendor.index'));
            }
            ...
            $row = new $this->spaceClass();
            $row->status = "publish";
            if(setting_item("property_sale_vendor_create_service_must_approved_by_admin", 0)){
                $row->status = "pending";
            }
        }
        $dataKeys = [
        ...
        if ($res) {
            ...
            if($id > 0 ){
                ...
                $row->uid = 'ps-' . $total_count;
                $row->save();
                
                event(new CreatedServicesEvent($row));
                return redirect(route('sale.vendor.edit',['id'=>$row->id]))->with('success', __('Sale created') );
            }
        }
    }
    ...
    public function editSpace(Request $request, $id)
    {
        ...
        $row = $row->find($id);
        if (empty($row)) {
            return redirect(route('sale.vendor.index'))->with('warning', __('Space not found!'));
        }
    ...
    public function deleteSpace($id)
    {
        ...
        return redirect(route('sale.vendor.index'))->with('success', __('Delete sale success!'));
    }
	...
	public function cloneSpace(Request $request,$id){
		...
		$row = $row->find($id);
		if (empty($row)) {
			return redirect(route('sale.vendor.index'))->with('warning', __('Sale not found!'));
		}
		try{
			$clone = $row->replicate();
			$clone->status  = 'draft';
			...
			return redirect()->back()->with('success',__('Sale clone was successful'));
