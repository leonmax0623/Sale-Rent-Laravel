...
class SpaceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($list)) {
            foreach ($list as $row) {
                $markers[] = [
                    ...
                    "infobox" => view('Sale::frontend.layouts.search.loop-grid', ['row' => $row,'disable_lazyload'=>1,'wrap_class'=>'infobox-item'])->render(),
        ...
        $layout = setting_item("property_sale_layout_search", 'normal');
        if ($request->query('_layout')) {
            $layout = $request->query('_layout');
        }
        if ($is_ajax) {
            return $this->sendSuccess([
                'html'    => view('Sale::frontend.layouts.search-map.list-item', $data)->render(),
                "markers" => $data['markers']
            ]);
        }
        $data['attributes'] = Attributes::where('service', 'space')->orderBy("position","desc")->with(['terms'=>function($query){
            $query->withCount('sale');
        },'translations'])->get();
        if ($layout == "map") {
            $data['body_class'] = 'has-search-map';
            $data['html_class'] = 'full-page';
            return view('Sale::frontend.search-map', $data);
        }
        return view('Sale::frontend.search', $data);
    }
    ...
    public function detail(Request $request, $property_type_slug, $uid, $slug)
    {
        ...
        return view('Sale::frontend.detail', $data);
    }
}
