...
use Modules\Sale\Models\SaleTranslation;
...
class Sale extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bc_sales';
    public $type = 'sale';
    public $moduleSlug = 'sale';
    public $checkout_booking_detail_file       = 'Sale::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Sale::frontend/booking/detail-modal';
    public $set_paid_modal_file                = 'Sale::frontend/booking/set-paid-modal';
    public $email_new_booking_file             = 'Sale::emails.new_booking_detail';
    public $availabilityClass = SaleDate::class;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->spaceDateClass = SaleDate::class;
        $this->spaceTermClass = SaleTerm::class;
        $this->spaceTranslationClass = SaleTranslation::class;
    ...
    {
        $meta['seo_title'] = __("Search for Sales");
        if (!empty($title = setting_item_with_lang("property_sale_page_list_seo_title",false))) {
            $meta['seo_title'] = $title;
        }else if(!empty($title = setting_item_with_lang("property_sale_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("property_sale_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        }else if(!empty($title = setting_item("property_sale_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("property_sale_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("property_sale_page_list_seo_share");
        $meta['full_url'] = url(config('space.sale_route_prefix'));
        return $meta;
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail .= config('space.sale_route_prefix') . $property_type_path . "/" . $this->uid . "/" . $this->slug;
        if(!empty($param)){
    ...
    public static function getLinkForPageSearch( $locale = false , $param = [] ){
        ...
        return url(app_get_locale(false , false , '/'). config('space.sale_route_prefix')."?".http_build_query($param));
    }
    ...
    {
        return url(route('sale.admin.edit',['id'=>$this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('property_sale_booking_buyer_fees'))) {
    ...
    public function beforeCheckout(Request $request, $booking)
    {
        if(!$this->isAvailableInRanges($booking->start_date,$booking->end_date)){
            return $this->sendError(__("This sale is not available at selected dates"));
        }
    }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('property_sale_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("property_sale_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("property_sale_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("property_sale_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("property_sale_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("property_sale_review_stats", []))) {
    ...
    public function getScoreReview()
    {
        ...
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $space_id, function () use ($space_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $space_id)->where('object_model', "sale")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])->where('object_id', $this->id)->where('object_model', 'space')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('property_sale_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Sales", ['number' => $number]);
        }
        return __(":number Sale", ['number' => $number]);
    }
    ...
    public static function isEnable(){
        return setting_item('property_sale_disable') == false;
    }

    public function isDepositEnable(){
        return (setting_item('property_sale_deposit_enable') and setting_item('property_sale_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('property_sale_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('property_sale_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('property_sale_deposit_fomular','default');
    }
    ...
    {
        //dump($request->all());
        $model_space = parent::query()->select("bc_sales.*");
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if(!empty($location)){
                $model_space->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_sales.location_id')
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bc_sales.sale_price,0) > 0 and bc_sales.sale_price >= ? ) OR (IFNULL(bc_sales.sale_price,0) <= 0 and bc_sales.price >= ? ) ) 
                            AND ( (IFNULL(bc_sales.sale_price,0) > 0 and bc_sales.sale_price <= ? ) OR (IFNULL(bc_sales.sale_price,0) <= 0 and bc_sales.price <= ? ) )";
        ...
        if ($request->query('sale_price_id')) {
            $sale_price = SalePrice::where('id', $request->query('sale_price_id'))->first();
            if($sale_price) {
                $raw_sql_min_max = "( (IFNULL(bc_sales.sale_price,0) > 0 and bc_sales.sale_price >= ? ) OR (IFNULL(bc_sales.sale_price,0) <= 0 and bc_sales.price >= ? ) ) 
                                AND ( (IFNULL(bc_sales.sale_price,0) > 0 and bc_sales.sale_price <= ? ) OR (IFNULL(bc_sales.sale_price,0) <= 0 and bc_sales.price <= ? ) )";
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_space->join('bc_sale_term as tt', 'tt.target_id', "bc_sales.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_sales.review_score >= ? AND bc_sales.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_space->leftJoin('bc_sale_translations', function ($join) {
                    $join->on('bc_sales.id', '=', 'bc_sale_translations.origin_id');
                });
                $model_space->where('bc_sale_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_space->where('bc_sales.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_space->orderByRaw("POW((bc_sales.map_lng-?),2) + POW((bc_sales.map_lat-?),2)",[$lgn,$lat]);
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_sales.sale_price, 0 ) > 0 THEN bc_sales.sale_price ELSE bc_sales.price END AS tmp_min_price";
        ...
        if(!empty($request->query('limit'))){
            ...
            $limit = !empty(setting_item("property_sale_page_limit_item"))? setting_item("property_sale_page_limit_item") : 9;
        }
    ...
    {
        return "\Modules\Sale\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("sale")
            ]
        ];
    }
    public static function getBookingType(){
        return setting_item('property_sale_booking_type','by_day');
    }

    static public function getFormSearch()
    {
        $search_fields = setting_item_array('property_sale_search_fields');
