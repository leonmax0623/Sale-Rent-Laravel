...
class SpaceTermFeaturedBox extends BaseBlock
{
        ...
        $this->setOptions([
            'settings' => [
                ...
                [
                    ...
                    'select2'      => [
                        'ajax'     => [
                            'url'      => route('sale.admin.attribute.term.getForSelect2', ['type' => 'space']),
                            'dataType' => 'json'
                        ],
                        'width'    => '100%',
                        'multiple' => "true",
                    ],
                    'pre_selected' => route('sale.admin.attribute.term.getForSelect2', [
                        'type'         => 'sale',
                        'pre_selected' => 1
                    ])
                ],
    ...
    {
        return __('Sale: Term Featured Box');
    }
    ...
    public function content($model = [])
    {
        ...
        $list_term = Terms::whereIn('id',$term_space)->with('space')->get();
        $model['list_term'] = $list_term;
        return view('Sale::frontend.blocks.term-featured-box.index', $model);
    }
