...
//            $folder .= sprintf('%04d', (int)$id / 1000) . '/' . $id . '/';
        ...
        try {
            ...
            if(count($resize ) > 0) {
                ...
                if($resize[0] == null and $resize[1] == null ){
                    Image::make($file)->crop($resize[2],$resize[3])->save(public_path( $link));
                    Image::make($file)->encode('webp', 90)->crop($resize[2],$resize[3])->save(public_path($linkWebP));
                }else {
                    Image::make($file)->resize($resize[0], $resize[1])->crop($resize[2],$resize[3])->save(public_path( $link));
                    Image::make($file)->encode('webp', 90)->resize($resize[0], $resize[1])->crop($resize[2],$resize[3])->save(public_path( $linkWebP ));
                }
    ...
    public static function checkResize($file, $group = "default", $avatar)
    {
        ...
        if (Auth::user()->getRoleNames()[0] != 'administrator') {
            ...
            if ($imagedata[0] >= 1024 and $imagedata[1] >= 853 and $imagedata[1] < 1000 and $imagedata[0] > $imagedata[1] ) {
                $x = round($imagedata[0] * 853 / $imagedata[1]);
                if($x < 1024) {
                    $x = 1024;
                    return  [$x, 800, 1024, 682];
                }
                return [$x, 853, 1280, 853];
            }

            if ($imagedata[0] >= 1024 and $imagedata[1] >= 800 and $imagedata[1] < 853 and $imagedata[0] > $imagedata[1] ) {
            ...
            if ($imagedata[0] < 1500 and $imagedata[0] >= 1280 and $imagedata[1] >= 1500 and $imagedata[0] < $imagedata[1]) {
                $y = round($imagedata[1] * 1280 / $imagedata[0]);
                return [1280, $y, 1280, 853];
            }

            if ($imagedata[0] < 1280  and $imagedata[0] >= 1024 and $imagedata[1] >= 1500 and $imagedata[0] < $imagedata[1]) {
                $y = round($imagedata[1] * 1280 / $imagedata[0]);
            ...
            if ($imagedata[0] < 1500 and $imagedata[1] < 1500 and $imagedata[0] >= 1280  and $imagedata[0] = $imagedata[1]) {
                return [1280, 1280, 1280, 800];
            }

            if ($imagedata[0] < 1280 and $imagedata[1] < 1280 and $imagedata[0] >= 1024  and $imagedata[0] = $imagedata[1]) {
